...
Release packages redistribute the pinned `rm-xovi-extensions`
...
Every binary release includes a versioned corresponding-source archive beside
the downloads. That archive contains:
...
The repository tag and upstream links are useful for review, but the source
archive is the downloadable copy that belongs with the release.
