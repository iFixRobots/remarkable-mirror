...
- For the current app, download `ReMarkableMirror-Windows-x64-portable.exe`
  from the [latest release](https://github.com/iFixRobots/remarkable-mirror/releases/latest)
  and verify it against the published `SHA256SUMS.txt`.
- Releases ship the portable app. Installer ZIPs come from a **Build Windows
  downloads** Actions run (for testing a newer, unreleased commit) or a local
  package build; the Actions artifact holds the versioned installer ZIP inside
  the GitHub download — extract both layers before running `Install.cmd`.
