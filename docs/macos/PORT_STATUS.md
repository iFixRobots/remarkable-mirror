...
| Connection selection and lifecycle | USB-first monitor with paired Wi-Fi fallback | Owner-started `Connect USB-C` or `Connect Wi-Fi`; USB-C owns a bounded same-cable wake-through-connect session and never selects Wi-Fi | Intentional Mac divergence; USB-C exercised, Wi-Fi unproved |
