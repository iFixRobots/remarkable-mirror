...
Start with [macOS setup](GETTING_STARTED.md).

## macOS blocks the app

In Finder, Control-click the app, choose **Open**, and confirm **Open**.
...
## Setup fails instantly on every attempt

macOS gates local network access per app. If connections to the tablet die
immediately while the cable and tablet are fine, check **System Settings >
Privacy & Security > Local Network** and make sure reMarkable Mirror is
allowed. The prompt normally appears during the first setup attempt — click
**Allow**.

## The tablet answers nothing over USB-C

A tablet that just restarted answers nothing until it has been unlocked once
past its passcode. Wake it, unlock it to the home screen, then retry. If the
Mac shows no `10.11.99.x` interface at all, unplug the USB-C cable and plug it
back in.

## Setup fails on an untested tablet software version

Setup is tested on the versions listed in
[Platform support](../PLATFORM_SUPPORT.md) and proceeds on others. A factory
reset can boot the tablet's other firmware slot and change the active version.
If setup fails on an untested version, copy the connection diagnostics from
the Help menu and open an issue or PR so support can be added.
