...
This is a known gap, not a hidden procedure: the removal path does not exist
yet, and it must exist before Mirror can call itself a supported release. Do
not improvise root-level removal commands from this page.

## What can be removed locally

Windows can remove the installed app through **Settings > Apps > Installed
apps**. The macOS app can remove its app bundle, and **Set Up
