...
Development MSIX packages are self-signed. Use the portable EXE when you want a
single file that does not install a package certificate.
...
The package command creates the arm64 DMG under `artifacts/macos/package/`.
...
- Keep the tablet model strict. Treat firmware and active root as observed
  identity, Mirror component versions as runtime prerequisites, and paired
  firmware/build values as install-only targets.
...
See [Releasing](RELEASING.md) for the build and upload checklist.
