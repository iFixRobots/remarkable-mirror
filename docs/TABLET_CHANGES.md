...
The installer checks the Paper Pro Move model, the exact paired software/build
install target, and the input devices before it changes the tablet. It modifies
only the active A/B root slot. Passive runtime recognition does not use that
install allowlist; it verifies the already installed capabilities. A firmware
update can activate the other slot without those components.
...
After a firmware or root-slot switch, do not blindly reinstall. First let
Mirror inspect the installed capabilities. If repair is required, confirm that
the release lists the observed software/build pair as an install target,
connect and unlock over USB, then choose **Repair Tablet Setup** in the matching
