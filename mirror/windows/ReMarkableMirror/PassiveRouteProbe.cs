...
internal sealed class PassiveRouteProbe
{
    ...
    private static void CloseGracefully(TcpClient client)
    {
        // An orderly FIN exchange. Rapid abortive RST closes can wedge the
        // tablet's dropbear listener until the tablet reboots.
        try
        {
    ...
    string XoviVersion)
{
    ...
    public bool MeetsRuntimeContract =>
        ...
        string.Equals(UsbConnectionPolicy, ExpectedUsbConnectionPolicy, StringComparison.Ordinal) &&
        TransportActive &&
        WakeEndpointHealthy &&
