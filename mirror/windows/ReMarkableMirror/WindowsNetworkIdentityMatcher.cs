...
internal static class WindowsNetworkIdentityMatcher
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var reading = await ReadCurrentAsync(remoteAddress, cancellationToken)
            .ConfigureAwait(false);
        if (reading is null)
        {
            return WindowsNetworkMatchResult.Unavailable;
        }
        return Guid.TryParse(reading.InterfaceId, out var currentInterfaceGuid) &&
            currentInterfaceGuid == expectedInterfaceGuid &&
            string.Equals(
                reading.NetworkIdentity,
                expectedNetworkIdentity,
    ...
    /// <summary>
    /// Reads the local interface and network identity that currently route to
    /// the remote address, or null when they cannot be read unambiguously.
    /// </summary>
    public static async Task<WindowsNetworkIdentityReading?> ReadCurrentAsync(
        string remoteAddress,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (fields.Length != 3 ||
                !Guid.TryParse(fields[1], out _) ||
                string.IsNullOrWhiteSpace(fields[2]))
            {
                return null;
            }
            return new WindowsNetworkIdentityReading(fields[1], fields[2]);
        }
...
internal sealed record WindowsNetworkIdentityReading(
    string InterfaceId,
