...
public sealed class DeviceProfileStore
{
    ...
    // When any Wi-Fi pairing field is present, the whole group must validate.
    private static bool HasValidWifiPairing(DeviceProfile profile)
    {
        if (profile.LastVerifiedWifiHost is null ||
            profile.PairedWindowsInterfaceId is null ||
            profile.PairedWindowsNetworkIdentity is null)
        {
            return false;
        }
        if (profile.LastVerifiedWifiHost.Length == 0 &&
            profile.PairedWindowsInterfaceId.Length == 0 &&
            profile.PairedWindowsNetworkIdentity.Length == 0)
        {
            return true;
        }
        return IsSafeNetworkHost(profile.LastVerifiedWifiHost) &&
