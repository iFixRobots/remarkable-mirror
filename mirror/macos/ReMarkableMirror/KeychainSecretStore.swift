...
struct KeychainBackend: Sendable {
    ...
    // The data protection keychain needs a signed application identifier.
    // Builds without that entitlement fall back to the file-based login
    // keychain. Only writes surface errSecMissingEntitlement — reads answer
    // errSecItemNotFound — so the probe must attempt a write.
    private static let dataProtectionKeychainIsAvailable: Bool = {
        var probe: [CFString: Any] = [
            kSecClass: kSecClassGenericPassword,
            kSecAttrService: "com.ifixrobots.ReMarkableMirror.keychain-probe",
    ...
    private static func serviceQuery(service: String) -> [CFString: Any] {
        ...
        if dataProtectionKeychainIsAvailable {
