...
actor SocketSSHBannerProbe: SSHBannerProbing {
    ...
    func probe(target: SSHBannerProbeTarget) async throws -> SSHBannerProbeResult {
        let result = try await worker.run { cancellation in
            try Self.blockingProbe(target: target, cancellation: cancellation)
        }
        if case .ssh = result {
            Self.log.info(
                "host=\(target.host, privacy: .public) interface=\(target.boundInterface ?? "-", privacy: .public) result=ssh"
            )
        } else {
            Self.log.error(
                "host=\(target.host, privacy: .public) interface=\(target.boundInterface ?? "-", privacy: .public) result=\(String(describing: result), privacy: .public)"
            )
        }
        return result
    }
    ...
    private static func blockingProbe(
        ...
        if let boundInterface = target.boundInterface {
            guard let binding = interfaceBinding(named: boundInterface) else {
                log.error("interface binding lookup failed for \(boundInterface, privacy: .public)")
                return .noRoute(.tcpUnavailable)
            }
        ...
        defer { gracefulClose(descriptor) }
    ...
    private static func probeSocket(
        ...
        if var interfaceIndex = interfaceBinding?.index {
            guard setsockopt(
                ...
                log.error("setsockopt IP_BOUND_IF failed errno=\(errno)")
                return .noRoute(.tcpUnavailable)
            }
        }
        ...
        if connectResult != 0 {
            guard errno == EINPROGRESS else {
                log.error("connect failed errno=\(errno)")
                return .noRoute(.tcpUnavailable)
            }
            ...
            guard getsockopt(descriptor, SOL_SOCKET, SO_ERROR, &socketError, &length) == 0,
                  socketError == 0 else {
                log.error("connect SO_ERROR=\(socketError)")
                return .noRoute(.tcpUnavailable)
            }
        }
        ...
        guard let local = socketEndpoint(descriptor, peer: false),
            ...
            log.error("endpoint verification failed after connect")
            return .noRoute(.tcpUnavailable)
        }
    ...
    // A graceful FIN keeps the tablet's SSH wrapper from accumulating
    // half-torn-down children the way repeated RST closes do.
    private static func gracefulClose(_ descriptor: Int32) {
...
struct PassiveRouteCapability: Equatable, Sendable {
    ...
    var meetsRuntimeContract: Bool {
        tabletModel == Self.expectedTabletModel &&
            probeVersion == Self.expectedProbeVersion &&
            transportVersion == Self.expectedTransportVersion &&
            transportSchema == Self.expectedTransportSchema &&
            usbConnectionPolicy == Self.expectedUSBConnectionPolicy &&
            transportActive &&
            wakeEndpointHealthy &&
            xoviVersion == Self.expectedXoviVersion
    }
