...
enum TabletWiFiPairingProbe {
    ...
    static let discoveryCommand = #"""
        set -eu
        saved_connections=$(nmcli -t -f TYPE connection show 2>/dev/null) || exit 90
        saved_count=$(printf '%s\n' "$saved_connections" | awk -F: '$1 == "802-11-wireless" || $1 == "wifi" { count++ } END { print count + 0 }') || exit 91
        active_connections=$(nmcli -t -f TYPE,DEVICE connection show --active 2>/dev/null) || exit 92
        active_count=$(printf '%s\n' "$active_connections" | awk -F: '($1 == "802-11-wireless" || $1 == "wifi") && $2 == "wlan0" { count++ } END { print count + 0 }') || exit 93
