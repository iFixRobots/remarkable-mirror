...
actor ConnectionCoordinator {
    ...
    private var lastPairingAttemptInstant: ContinuousClock.Instant?
    ...
    func prepareLocalPairing() async -> Bool {
        ...
        // Floor between attempts so rapid retries cannot flood the tablet's
        // SSH service with connection churn.
        if let lastAttempt = lastPairingAttemptInstant {
            let elapsed = ContinuousClock.now - lastAttempt
            if elapsed < .seconds(1.5) {
                try? await Task.sleep(for: .seconds(1.5) - elapsed)
            }
            guard pairingOperation == nil, !isShuttingDown, !isStoppingActiveWork else {
                return false
            }
        }
        lastPairingAttemptInstant = ContinuousClock.now
    ...
    private static func usbRepairDisposition(
        ...
        case let .prerequisiteInstallFailed(failure):
            ...
            if failure.needsUnsupportedSoftwareAttention {
                return disposition.withManualEvidence(.unsupportedTabletSoftware)
            }
    ...
    private func runPairingOperation(id: UUID) async {
        ...
        Self.setupLog.error(
            "pairing prepare result=\(String(describing: result), privacy: .public)"
        )
        guard isCurrentPairingOperation(id), !Task.isCancelled, !isShuttingDown else {
            return
        }
    ...
    private func observeUSB(
        ...
        if result.state == .authenticated,
           result.capability?.meetsRuntimeContract == true,
        ...
        switch result.state {
        case .authenticated
            where result.capability?.meetsRuntimeContract == true &&
                result.capability?.transportOperational == true:
    ...
    private func observeWiFi(
        ...
        switch result.state {
        case .authenticated where result.capability?.meetsRuntimeContract == true:
