...
enum ConnectionPresentationState: Equatable, Sendable {
    ...
    var statusText: String {
        ...
        case .setupAttention, .setupPackageAttention, .xoviAttention,
             .tabletInstallAttention, .unsupportedTabletSoftware,
             .profileAttention,
             .attention: "Attention"
        ...
        case .repair: "Repair"
        case let .live(route): route == .usb ? "Live over USB" : "Live over Wi‑Fi"
        }
    }
    ...
    var recoveryCard: RecoveryCardContent? {
        ...
        case .setupWaitingForWakeAndUnlock:
            RecoveryCardContent(
                ...
                message: "The tablet isn’t answering over USB‑C. Wake it, enter its passcode, and try again — after a restart the tablet can’t answer until it has been unlocked once.",
                showsProgress: false,
                actions: [.retrySetup, .cancelSetup]
            )
        case .setupAttention:
        ...
        case .unsupportedTabletSoftware:
            RecoveryCardContent(
                symbol: "exclamationmark.triangle",
                title: "Tablet software isn't supported for setup",
                message: "This tablet's software version hasn't been set up by this Mirror build before. Copy the connection diagnostics from the Help menu and open an issue or PR so support can be added.",
                showsProgress: false,
                actions: []
            )
        case .awaitingTabletAuthorization:
        ...
        case let .awaitingTabletAuthorizationCheck(failureDescription):
            if let failureDescription {
                ...
                RecoveryCardContent(
                    ...
                    message: "Mirror checks the saved key, then installs or verifies its tablet components over USB‑C. It will not ask for the password again.",
                    showsProgress: false,
        ...
        case .awaitingWiFiVerification:
            RecoveryCardContent(
                ...
                message: "Keep USB‑C connected. Mirror finishes setup over the cable, then returns to the connection chooser.",
                showsProgress: false,
        ...
        case .readyToConnect:
            RecoveryCardContent(
                ...
                message: "Choose USB‑C or Wi‑Fi. Mirror will handle waking the tablet during a USB‑C connection. Backed-up documents can be put back with Connection > Restore Backup…",
                showsProgress: false,
...
enum ConnectionPublicationGate {
    static func presentation(for snapshot: ConnectionSnapshot) -> ConnectionPresentationState {
        ...
        case .unsupportedTabletSoftware: .unsupportedTabletSoftware
