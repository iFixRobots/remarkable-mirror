...
test "$(wc -l < "$stage/rmmirror-prerequisites.env")" -eq 10
if LC_ALL=C grep -Ev '^[A-Z0-9_]+=[A-Za-z0-9.,/+_-]+$' \
    "$stage/rmmirror-prerequisites.env" | grep -q .; then
  exit 1
fi
. "$stage/rmmirror-prerequisites.env"
test "${RMMIRROR_PREREQUISITES_SCHEMA:-}" = 'rmmirror.prerequisites/v2'
test "${RMMIRROR_TABLET_MODEL:-}" = 'chiappa'
LC_ALL=C printf '%s\n' "${RMMIRROR_TABLET_INSTALL_TARGETS:-}" | grep -Eq \
  '^[0-9]+([.][0-9]+){3}[+][0-9]+([.][0-9]+){2}(,[0-9]+([.][0-9]+){3}[+][0-9]+([.][0-9]+){2})*$'
...
LC_ALL=C printf '%s\n' "$tablet_img_version" | grep -Eq '^[0-9]+([.][0-9]+){3}$'
LC_ALL=C printf '%s\n' "$tablet_os_build" | grep -Eq '^[0-9]+([.][0-9]+){2}$'
tablet_install_target="${tablet_img_version}+${tablet_os_build}"
case ",${RMMIRROR_TABLET_INSTALL_TARGETS}," in
  *,$tablet_install_target,*) ;;
  *)
    printf '%s\n' "rmmirror-prerequisite: tablet_software_untested:${tablet_install_target}" >&2
    ;;
esac
failure_phase=validating_tablet
test -c /dev/uinput
