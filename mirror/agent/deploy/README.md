...
model, paired install-target allowlist, schema, component versions, Xovi hash,
transport policy, and extension-set contract. The allowlist is enforced only
before installation or repair; passive runtime readiness uses the installed
capability contract instead of firmware equality.
...
**Repair Tablet Setup** in a host build whose paired install targets include
the active software. Automatic installation on a newly active root slot is not
implemented.
