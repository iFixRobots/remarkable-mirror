...
pinned_toolchain=${${expected_go_version#go version }%% *}
...
actual_go_version=$(GOTOOLCHAIN=local "${go_command}" version)
if [[ "${actual_go_version}" != "${expected_go_version}" ]]; then
    # Run the pinned toolchain even when the host Go differs. GOTOOLCHAIN
    # provisions it here, and the isolated builds below invoke that exact
    # toolchain binary with GOTOOLCHAIN=local so nothing re-selects.
    if ! GOTOOLCHAIN=${pinned_toolchain} "${go_command}" version > /dev/null; then
        print -u2 "Could not provision the pinned Go toolchain '${pinned_toolchain}' with the host Go at ${go_command}."
        exit 1
    fi
    pinned_go_root=$(GOTOOLCHAIN=${pinned_toolchain} "${go_command}" env GOROOT)
    if [[ -z "${pinned_go_root}" || ! -x "${pinned_go_root}/bin/go" ]]; then
        print -u2 "Could not resolve GOROOT for the pinned Go toolchain '${pinned_toolchain}'."
        exit 1
    fi
    go_command="${pinned_go_root}/bin/go"
    actual_go_version=$(GOTOOLCHAIN=local "${go_command}" version)
fi
if [[ "${actual_go_version}" != "${expected_go_version}" ]]; then
