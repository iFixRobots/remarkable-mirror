...
rmmirror_validate_contract() {
    ...
    if [[ $(/usr/bin/wc -l < "${contract_path}") -ne 11 ]]; then
        print -u2 "The prerequisite contract must contain exactly eleven newline-terminated fields."
        return 1
    fi
    ...
    if (( ${#lines} != ${#expected_keys} )); then
        print -u2 "The prerequisite contract must contain exactly eleven fields."
        return 1
    fi
    ...
    [[ ${lines[1]#*=} == rmmirror.prerequisites/v1 ]] || return 1
    [[ ${lines[2]#*=} == chiappa ]] || return 1
    print -r -- "${lines[3]#*=}" | LC_ALL=C /usr/bin/grep -Eq '^[0-9]+[.][0-9]+[.][0-9]+[.][0-9]+$' || return 1
    print -r -- "${lines[4]#*=}" | LC_ALL=C /usr/bin/grep -Eq '^[0-9]+[.][0-9]+[.][0-9]+$' || return 1
    print -r -- "${lines[5]#*=}" | LC_ALL=C /usr/bin/grep -Eq '^v[0-9]+-[0-9]{8}$' || return 1
    rmmirror_is_sha256 "${lines[6]#*=}" || return 1
    print -r -- "${lines[7]#*=}" | LC_ALL=C /usr/bin/grep -Eq '^[0-9]+[.][0-9]+[.][0-9]+$' || return 1
    print -r -- "${lines[8]#*=}" | LC_ALL=C /usr/bin/grep -Eq '^[0-9]+[.][0-9]+[.][0-9]+$' || return 1
    print -r -- "${lines[9]#*=}" | LC_ALL=C /usr/bin/grep -Eq '^rmmirror[.]transport-wake/v[0-9]+$' || return 1
    print -r -- "${lines[10]#*=}" | LC_ALL=C /usr/bin/grep -Eq '^[a-z0-9-]+/v[0-9]+$' || return 1
    [[ ${lines[11]#*=} == framebuffer-spy.so,xovi-message-broker.so,rmmirror-files-loopback.so ]] || return 1
}
